...
if __name__ == '__main__':
    ...
    models = ['neuralwave', 'earthformer', 'unet']
    models_label = ['NWM', 'Earthformer', 'UNet']
    idn = 1.0
    ...
    for model in models:
        fpath = f'results/{model}/Progressive_Data_Ablation_Test/predictions_{idn_str}.nc'
        ...
        with nc.Dataset(fpath) as f:
            ...
            num_samples = swh_pred_all[model].shape[0]
            num_timesteps = swh_pred_all[model].shape[1]
            if any(si >= num_samples for si in sample_indices):
                print(f"Warning: One or more sample_indices ({sample_indices}) might be out of bounds for model {model} (num_samples: {num_samples})")
            if t_idx >= num_timesteps:
                 print(f"Warning: t_idx ({t_idx}) might be out of bounds for model {model} (num_timesteps: {num_timesteps})")
