...
if __name__ == '__main__':
    ...
    models = ['NWM', 'EF', 'UNet']
    path_to_model = {'NWM': 'neuralwave', 'EF': 'earthformer', 'UNet': 'unet'}
    idn = 0.1
    idn_str = f'{idn:.1f}'

    sample_indices = [6, 46]
    t_idx = 30
    sample_steps = [(ci, t_idx) for ci in sample_indices]

    swh_pred_all, swh_true_all = {}, {}
    mwd_pred_all, mwd_true_all = {}, {}
    x = None
    for model in models:
        fpath = f'results/{path_to_model[model]}/Noise_Injected_Robustness_Test/predictions_{idn_str}.nc'
        with nc.Dataset(fpath) as f:
            ...
            mwd_true_all[model] = np.array(
                ...
                dtype=np.float32)
            x = np.array(f.variables['x'][:], dtype=np.float32)

    plot_param_compare(models, swh_pred_all, swh_true_all, x,
                       sample_steps, 'SWH', 'SWH (m)',
                       y_limits=(0.5, 1.0),
                       savepath=f'{figures_path}/Figure_13a.tiff')
    plot_param_compare(models, mwd_pred_all, mwd_true_all, x,
                       sample_steps, 'MWD', 'MWD (°)',
                       y_limits=None,
                       savepath=f'{figures_path}/Figure_13b.tiff')
