...
if __name__ == "__main__":
    ...
    models = ['neuralwave', 'earthformer', 'unet', 'nwm_dynamic']
    model_labels = ['NWM', 'Earthformer', 'UNet', 'NWM(Dynamic Module)']
    ...
    for col_idx, model_name in enumerate(models):
        ...
        file_path = f'results/{model_name}/Progressive_Data_Ablation_Test/predictions_0.1.nc'
        ...
        plot_scatter('swh', swh_pred_masked, swh_true_masked,
                     ax=axs[0, col_idx], fig=fig, subplot_title_prefix=model_labels[col_idx])
        plot_scatter('mwd', mwd_pred_masked, mwd_true_masked,
                     ax=axs[1, col_idx], fig=fig, subplot_title_prefix=model_labels[col_idx])

    fig.tight_layout(rect=[0, 0.03, 1, 0.95])

    plt.savefig(f'{figures_path}/Figure_4b.tiff', dpi=300, format='tiff', bbox_inches='tight')
